...
async function savePuzzleCompletion() {
  ...
  try {
    const localCompleted = JSON.parse(localStorage.getItem('cutie_puzzle_completed') || '[]');
    if (!localCompleted.includes(puzzleImageFilename)) {
      localCompleted.push(puzzleImageFilename);
      localStorage.setItem('cutie_puzzle_completed', JSON.stringify(localCompleted));
    }
  } catch(e) {}
}
